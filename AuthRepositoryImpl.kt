...
class AuthRepositoryImpl @Inject constructor(
    ...
    override suspend fun updateProfile(
        ...
        return try {
            val success = apiService.updateProfile(name)
            if (success) {
                // Update nama di local storage jika perlu
                sharedPreferences.edit().putString("USER_NAME", name).apply()
                Resource.Success(true)
            } else {
                Resource.Error("Gagal mengupdate profil")
            }
        } catch (e: Exception) {
    ...
    override suspend fun changePassword(
        ...
        return try {
            val success = apiService.changePassword(currentPass, newPass)
            if (success) Resource.Success(true) else Resource.Error("Password lama salah")
        } catch (e: Exception) {
            Resource.Error(e.localizedMessage ?: "Error")
        }
    }
